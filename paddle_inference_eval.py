...
def argsparser():
    ...
    parser.add_argument(
        "--model_path", type=str, default='ppyolo_r18vd_ptq', help="inference model filepath")
    parser.add_argument(
        "--image_file",
        type=str,
        default='./cells_coco/TEST/cell_320_0000.JPG',
    ...
    parser.add_argument(
        "--cpu_threads", type=int, default=None, help="Num of cpu threads.")

    parser.add_argument("--img_shape", type=int, default=320, help="input_size")
...
def predict_image(predictor,
    ...
    for i in range(repeats):
        ...
        inputs = {}
        inputs["image"] = img
        inputs['im_shape'] = np.array(image_shape).reshape((1, 2)).astype(np.float32)
        if include_nms:
        ...
        time_end = time.time()
        preprocess_time = round((time_end - time_start)*1000,2)
        time_log.append(preprocess_time)
        time_min = min(time_min, preprocess_time)
        time_max = max(time_max, preprocess_time)
        time_all += preprocess_time
    # print("preprocess cost :{} ms,min:{},max:{}".format(time_all,time_min,time_max))
    ...
    for i in range(repeats):
        ...
        end_time = time.time()
        timed = round((end_time - start_time)*1000,2)
        time_predict_log.append(timed)
    ...
    print("[Benchmark]Inference time(ms): min={}, max={}, avg={}".format(
        round(time_min, 2),
        round(time_max , 2), round(time_avg, 2)))
    print("preprocess cost :{} ms,min:{},max:{}".format(time_all, time_min, time_max))
    print("every img preprocess cost:{},----min:{}" .format(time_log,min(time_log)))
    print("=======================================================")
    print("every img predict cost:{},----min:{}" .format(time_predict_log,min(time_predict_log)))
...
def eval(predictor, val_loader, metric, rerun_flag=False):
    ...
    for batch_id, data in enumerate(val_loader):
        ...
        for i, _ in enumerate(input_names):
            ...
            timed_read = end_time_read - start_time_read
            time_min_read = min(time_min, timed_read)
            time_max_read = max(time_max, timed_read)
            read_time += timed_read

            start_time_copy = time.time()
            input_tensor.copy_from_cpu(data_all[input_names[i]])
            end_time_copy = time.time()
            timed_copy = end_time_copy - start_time_copy
            time_min_copy = min(time_min, timed_copy)
            time_max_copy = max(time_max, timed_copy)
            copy_time += timed_copy
    ...
    copy_avg = copy_time / sample_nums

    print("[Benchmark]Avg cpu_mem:{} MB, avg gpu_mem: {} MB".format(
        cpu_mems / sample_nums, gpu_mems / sample_nums))
    print("[Benchmark]Inference time(ms): min={}, max={}, avg={}".format(
        round(time_min * 1000, 4),
        round(time_max * 1000, 4), round(time_avg * 1000, 4)))

    print("[Benchmark]Read image time(ms): min={}, max={}, all={}".format(
        round(time_min_read * 1000, 4),
        round(time_max_read * 1000, 4), round(read_time * 1000, 4)))

    print("[Benchmark]Copy tensor to GPU time(ms): min={}, max={}, avg={}".format(
        round(time_min_copy * 1000, 4),
        round(time_max_copy * 1000, 4), round(copy_avg * 1000, 4)))
